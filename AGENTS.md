...
- agent 新建 Markdown 文件只能通过 `vault_create_note` 执行；工具必须拒绝覆盖已有文件或目录，只允许创建用户可见的 `.md` 文件，并且最多自动递归创建 3 层父目录
- agent 修改已有 Markdown 文件只能通过 `vault_edit_note` 执行精确替换、删除、插入或范围替换，写入前必须校验 `vault_read_note` 返回的 `contentHash`
...
- 遇到较难复现或定位的 bug 时，可临时添加 `console` 输出辅助调试，并引导用户截图关键输出结果进行反馈
- bug 修复完成后，必须删除本次排障过程中添加的所有临时调试代码（包括 `console` 输出与临时分支判断）

## 发布版本约定

- 发布版本号以 `package.json` 的 `version` 为唯一手动修改入口
- `src-tauri/tauri.conf.json` 的 `version` 必须保持为 `../package.json`，由 Tauri 读取 `package.json` 中的版本号
- GitHub Actions 的 Release workflow 会按 `package.json` 版本自动创建 `v<version>` tag 和 Release 草稿
