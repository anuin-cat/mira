...
/** 读取并校验回收站元数据 */
async function readTrashMetadata(itemId: string): Promise<TrashMetadata | null> {
  if (!SAFE_TRASH_ITEM_ID_RE.test(itemId)) return null
  const raw = await readFile(joinRelativePath(TRASH_ROOT_DIR, itemId, TRASH_METADATA_FILE))
  if (!raw) return null
...
/** 找到不会冲突的回收站批次目录 */
async function resolveAvailableTrashItemId(date: Date): Promise<string> {
  while (true) {
    const itemId = createTrashBatchId(date)
    const exists = await pathExists(joinRelativePath(TRASH_ROOT_DIR, itemId))
...
export async function moveEntryToTrash(
  ...
  const sourceInfo = await getFileInfo(normalizedPath)
  ...
  const itemId = await resolveAvailableTrashItemId(new Date(deletedAt))
  ...
  if (targetParentPath) await ensureDir(targetParentPath)

  const metadata: TrashMetadata = { id: itemId, originalPath: normalizedPath, kind, deletedAt }
  await writeFile(joinRelativePath(itemDir, TRASH_METADATA_FILE), JSON.stringify(metadata, null, 2))

  // 3. 移动原始内容；失败时移除空批次，避免留下不可恢复条目
  try {
    await movePath(normalizedPath, targetPath)
  } catch (error) {
    await removePath(itemDir, true)
    throw error
  }
...
/** 列出仍有有效元数据和内容的回收站条目 */
export async function listTrashItems(_vaultPath: string): Promise<TrashItem[]> {
  // 1. 只读取 trash 根目录下一层批次目录
  const entries = await listEntries(TRASH_ROOT_DIR)
  const items: TrashItem[] = []

  // 2. 元数据无效或内容丢失的目录不显示，也不自动删除
  for (const entry of entries) {
    if (!entry.name || !entry.isDirectory || entry.isSymlink) continue
    const metadata = await readTrashMetadata(entry.name)
    const item = metadata ? createTrashItem(metadata) : null
    if (!item) continue

    const contentExists = await pathExists(joinRelativePath(TRASH_ROOT_DIR, item.id, item.originalPath))
...
export async function restoreTrashItem(_vaultPath: string, itemId: string): Promise<TrashItem> {
  // 1. 读取元数据并检查来源、目标路径
  const metadata = await readTrashMetadata(itemId)
  const item = metadata ? createTrashItem(metadata) : null
  ...
  const targetExists = await pathExists(metadata.originalPath)
  if (targetExists) throw new Error('原位置已有同名文件或文件夹，无法恢复')

  // 2. 自动补齐原父目录，然后移动回原路径
  const targetParentPath = getParentPath(metadata.originalPath)
  if (targetParentPath) await ensureDir(targetParentPath)
  await movePath(sourcePath, metadata.originalPath)

  // 3. 清理回收站批次目录中残留的空父目录和元数据
  await removePath(joinRelativePath(TRASH_ROOT_DIR, metadata.id), true)
  return item
}

/** 永久删除单个回收站批次 */
export async function deleteTrashItem(_vaultPath: string, itemId: string): Promise<void> {
  const safeItemId = assertTrashItemId(itemId)
  await removePath(joinRelativePath(TRASH_ROOT_DIR, safeItemId), true)
}
